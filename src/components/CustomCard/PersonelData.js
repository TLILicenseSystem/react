        ...
        <Col xs="12" sm="5">
          <Row style={{ marginBottom: "0" }}>
            ...
            <Col sm="8" style={{padding :'0'}} >
              ...
              readOnly={true} value={`${get(data, "firstName", "")} ${get(data, "lastName", "")}`} />
            </Col>
          </Row>
        </Col>
        <Col >
          <Row style={{ marginBottom: "0" }}>
            ...
            <Col sm="7" style={{paddingRight :'0'}}>
              <Input
                id="strid"
                type={"tel"}
                value={get(data, "highStrid", "").length === 3
                ? get(data, "highStrid", "").split("").join("-")
                :get(data, "highStrid", "").length === 5 
                ? get(data, "highStrid", "").slice(0, 1) +
                  "-" +
                  get(data, "highStrid", "").slice(1, get(data, "highStrid", "").length - 1) +
                  "-" +
                  get(data, "highStrid", "").slice(get(data, "highStrid", "").length - 1)
